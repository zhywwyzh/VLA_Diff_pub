...
def visualize_trajectories_3d(traj1, traj2, prompt, save_path):
    """
    使用 matplotlib 可视化两条3D轨迹并保存到文件。
    """
    pos1 = traj1[:, :3]
    pos2 = traj2[:, :3]
    ...
    fig.suptitle(f"Task: {prompt}", fontsize=14, y=0.92, wrap=True)
    ax.legend()
    plt.savefig(save_path, bbox_inches='tight')
    plt.close(fig)

def visualize_trajectories_2d_topdown(traj1, traj2, prompt, save_path):
    """
    使用 matplotlib 可视化两条轨迹的俯视图并保存到文件。
    横轴为Y (左/右), 纵轴为X (前/后)
    """
    ...
    ax.set_title(f'Trajectory Comparison (Top-down View)\nTask: {prompt}', wrap=True)
    ax.legend()
    ax.grid(True)
...
def main():
    ...
    base_path = "/data/vla/uav_flow_lerobot_3w_final/test/uav_flow"
    # case_name = "pass"
    ...
    case_name = '1w_dataset'
    # case_name = '3w_dataset'
    ...
    for i in range(17): # 循环 chunk-000 到 chunk-016
        chunk_name = f"chunk-{i:03d}"
        parquet_dir = os.path.join(base_path, "data", chunk_name)

        if not os.path.isdir(parquet_dir):
            logging.warning(f"目录未找到，跳过: {parquet_dir}")
            continue
        
        logging.info(f"正在处理目录: {parquet_dir}")
        
        # 获取并处理每个 chunk 目录下的前5个 parquet 文件
        parquet_files = sorted([f for f in os.listdir(parquet_dir) if f.endswith(".parquet")])[:5]
        ...
        for filename in parquet_files:
            ...
            try:
                ...
                output_dir_for_images = f"/data/vla/VLA_Diff/Openpi/test/infer/infer_output{case_name}"
                ...
                try:
                    ...
                    base_path_for_meta = os.path.dirname(os.path.dirname(os.path.dirname(EPISODE_DATA_PATH)))
                    tasks_jsonl_path = os.path.join(base_path_for_meta, 'meta', 'tasks.jsonl')
                ...
                if episode_match:
                    ...
                    base_filename = os.path.splitext(os.path.basename(EPISODE_DATA_PATH))[0]

                # 可视化轨迹
                logging.info("正在生成轨迹对比图...")
                plot_output_dir = f"/data/vla/VLA_Diff/Openpi/test/infer/trajectory_plots/{case_name}"
                os.makedirs(plot_output_dir, exist_ok=True)
                
                save_path_3d = os.path.join(plot_output_dir, f"{base_filename}_3d.png")
                save_path_2d = os.path.join(plot_output_dir, f"{base_filename}_2d.png")

                visualize_trajectories_3d(original_trajectory, inferred_trajectory, PROMPT, save_path_3d)
                visualize_trajectories_2d_topdown(original_trajectory, inferred_trajectory, PROMPT, save_path_2d)
                logging.info(f"轨迹对比图已保存到: {plot_output_dir}")

                # 从EPISODE_DATA_PATH动态生成输出JSON文件名
                if episode_match:
                    output_filename = f"inferred_trajectory_{episode_num}.json"
                else:
                    output_filename = f"inferred_trajectory_{base_filename}.json" # Fallback

                output_path = f"/data/vla/VLA_Diff/Openpi/test/infer/trajectory_video/{case_name}/{output_filename}"
                ...
                # try:
                #     generate_video_from_trajectory(output_path, case_name)
                #     logging.info("视频生成完毕。")
                # except Exception as e:
