...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    // ADD
    emergency_flag_sub_ = nh.subscribe("/emergency_flag", 1, &EGOReplanFSM::emergencyFlagCallback, this);
  ...
  // ADD
  void EGOReplanFSM::emergencyFlagCallback(const std_msgs::Bool::ConstPtr &msg) {
    emergency_flag_ = msg->data;
    if (emergency_flag_) {
        ROS_WARN("Emergency flag received! Triggering emergency stop.");
        changeFSMExecState(EMERGENCY_STOP, "EMERGENCY_FLAG");
        flag_escape_emergency_ = true;
    }
  }
  ...
  bool EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    // ADD
    historical_waypoints_.push_back(next_wp);
    visualization_->displayGoalPathList(historical_waypoints_, 0.2, 0);
  ...
  // ADD新增回调函数
  void EGOReplanFSM::uavActionCallback(const geometry_msgs::PoseStampedConstPtr &msg) {
    if (msg->pose.position.z < -10) return;
