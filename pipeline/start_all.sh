...
if [ -f "requirements.txt" ]; then
    ...
    # 检查ChatGLM语音SDK
    log_info "检查ChatGLM语音SDK..."
    python3 -c "import zai; print('ChatGLM语音SDK已安装')" 2>/dev/null && log_success "ChatGLM语音SDK可用" || log_warning "ChatGLM语音SDK未安装，将使用备用TTS"
...
# 端口配置 - 如需修改请同时更新相关文件
MAIN_PORT=1024        # 主服务端口 (对应 app.py 中的 MAIN_SERVICE_PORT)
EXTERNAL_PORT=3000    # 外部接收器端口 (对应 external_receiver_example.py 中的 EXTERNAL_RECEIVER_PORT)
INCOMING_PORT=3001    # 外部消息接收端口 (对应 incoming_receiver.py 中的 INCOMING_RECEIVER_PORT)
...
check_port $INCOMING_PORT "外部消息接收器"
...
echo -e "  • 外部消息接收端口:   ${CYAN}$INCOMING_PORT${NC}"
echo -e "  • 主界面地址:         ${CYAN}http://localhost:$MAIN_PORT${NC}"
echo -e "  • 外部接收器:         ${CYAN}http://localhost:$EXTERNAL_PORT${NC}"
echo -e "  • 外部消息接收器:     ${CYAN}http://localhost:$INCOMING_PORT${NC}"
echo -e "  • 转发URL设置:        ${CYAN}http://localhost:$EXTERNAL_PORT/api/receive${NC}"
echo -e "  • 发送消息URL:        ${CYAN}http://localhost:$INCOMING_PORT/api/send${NC}"
echo ""

# 询问是否启动外部服务
read -p "$(echo -e ${YELLOW}"是否启动外部接收器？(Y/n): "${NC})" start_external
start_external=${start_external:-Y}

read -p "$(echo -e ${YELLOW}"是否启动外部消息接收器？(Y/n): "${NC})" start_incoming
start_incoming=${start_incoming:-Y}
...
INCOMING_LOG="logs/incoming_receiver.log"
...
if [[ $start_external =~ ^[Yy]$ ]]; then
    log_info "启动外部接收器 (端口: $EXTERNAL_PORT)..."
    # 确保使用虚拟环境
    source ../venv/bin/activate
...
# 启动外部消息接收器（如果用户选择）
if [[ $start_incoming =~ ^[Yy]$ ]]; then
    log_info "启动外部消息接收器 (端口: $INCOMING_PORT)..."
    # 确保使用虚拟环境
    source ../venv/bin/activate
    nohup python3 incoming_receiver.py > "$INCOMING_LOG" 2>&1 &
    INCOMING_PID=$!
    ...
    # 检查外部消息接收器是否启动成功
    if kill -0 $INCOMING_PID 2>/dev/null; then
        log_success "外部消息接收器启动成功 (PID: $INCOMING_PID)"
        echo "$INCOMING_PID" > logs/incoming_receiver.pid
    else
        log_error "外部消息接收器启动失败，请检查日志: $INCOMING_LOG"
    fi
fi
