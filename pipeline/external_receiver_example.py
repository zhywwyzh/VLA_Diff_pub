...
def index():
    ...
    <body>
        ...
        <h3>📨 最近消息</h3>
        {"".join([f'<div class="message"><strong>#{msg["id"]}</strong> ({msg["received_at"][:19]})<br>{msg["text"][:100]}{"..." if len(msg["text"]) > 100 else ""}</div>' for msg in received_messages[-5:]])}
...
if __name__ == '__main__':
    ...
    print("📍 接收端点: http://localhost:3000/api/receive")
    print("📍 状态页面: http://localhost:3000")
    print("💡 在Gemini聊天系统中设置转发URL为: http://localhost:3000/api/receive")
    print("-" * 60)
