...
def main(
    ...
    # OPTIMIZATION: Set pin_memory=True for faster CPU->GPU transfers
    # OPTIMIZATION: Increase num_workers to parallelize data loading
    ...
    for batch in tqdm.tqdm(data_loader, total=num_batches, desc="Computing stats on GPU"):
        for key in keys:
            ...
            # Data is already on its way to the GPU thanks to pin_memory
            values = batch[key].to(torch_device, non_blocking=True).float()
