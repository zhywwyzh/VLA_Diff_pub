...
{
  // ADD
  PolyTrajOptimizer::PolyTrajOptimizer() : nh_("~") {
    emergency_flag_pub_ = nh_.advertise<std_msgs::Bool>("/emergency_flag", 1);
  }
    ...
    for (int i = 1; i <= i_end; ++i)
    {
      ...
      if (occ)
      {
        ...
        if (j >= cps_.cp_size) // fail to get the obs free point
        {
          ...
          emergency_flag_pub_.publish(emergency_msg);
          last_emergency_state_ = true;

          force_stop_type_ = STOP_FOR_ERROR;
          return false;
        }
        else if (last_emergency_state_){
          // 如果上一次是紧急状态，但这次没有碰撞，发布 false
          std_msgs::Bool emergency_msg;
