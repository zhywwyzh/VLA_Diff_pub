...
namespace ego_planner
{
  PlanningVisualization::PlanningVisualization(ros::NodeHandle &nh)
  {
    ...
    // ADD
    goal_list_pub = nh.advertise<visualization_msgs::Marker>("goal_list", 2);
  ...
  // ADD
  void PlanningVisualization::displayGoalPathList(vector<Eigen::Vector3d> init_pts, const double scale, int id)
  {

    if (goal_list_pub.getNumSubscribers() == 0)
    {
      return;
    }

    Eigen::Vector4d color(0, 0.5, 0.5, 1);
    displayMarkerList(goal_list_pub, init_pts, scale, color, id);
  }
