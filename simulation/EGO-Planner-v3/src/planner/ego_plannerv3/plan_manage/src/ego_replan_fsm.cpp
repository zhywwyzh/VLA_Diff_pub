...
namespace ego_planner
{
  ...
  bool EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp, const double next_yaw, const bool look_forward)
  {
    final_goal_ = next_wp;
    ...
    if (exec_state_ == WAIT_TARGET)
    {
      ...
      if (dxy.norm() > 0.1 && look_forward ) // 0.1m
      {
        ...
        traj_server_.setYaw(des_yaw, atan2(M(1, 0), M(0, 0)), odom_pos_, true);
      }
      ...
      if (!look_forward)
      {
        ...
        Eigen::Matrix3d M = odom_q_.matrix();
        traj_server_.setYaw(next_yaw, atan2(M(1, 0), M(0, 0)), odom_pos_, false);
      }
    }
